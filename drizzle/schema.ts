...
export const purchaseOrders = mysqlTable("purchaseOrders", {
  ...
  locationId: int("locationId").notNull(),
  status: mysqlEnum("status", ["draft", "pending_approval", "approved", "submitted", "partially_received", "received", "cancelled"]).default("draft"),
  orderDate: date("orderDate"),
  expectedDate: date("expectedDate"),
  subtotal: decimal("subtotal", { precision: 12, scale: 2 }).default("0.00"),
  gst: decimal("poGst", { precision: 10, scale: 2 }).default("0.00"),
  qst: decimal("poQst", { precision: 10, scale: 2 }).default("0.00"),
  total: decimal("poTotal", { precision: 12, scale: 2 }).default("0.00"),
  notes: text("notes"),
  createdByPin: int("createdByPin"),
  approvedByPin: int("approvedByPin"),
...
export const revenueJournalEntries = mysqlTable("revenueJournalEntries", {
  ...
  gst: decimal("gst", { precision: 10, scale: 2 }).default("0.00"),
  qst: decimal("qst", { precision: 10, scale: 2 }).default("0.00"),
  status: mysqlEnum("revenueJeStatus", ["posted", "voided", "deleted", "failed", "pending"]).default("pending").notNull(),
  ...
  errorMessage: text("errorMessage"),
  jeLineDetails: json("jeLineDetails"),
  taxExemptSales: decimal("taxExemptSales", { precision: 12, scale: 2 }).default("0.00"),
  taxableSales: decimal("taxableSales", { precision: 12, scale: 2 }).default("0.00"),
  tips: decimal("tips", { precision: 10, scale: 2 }).default("0.00"),
  pettyCash: decimal("pettyCash", { precision: 10, scale: 2 }).default("0.00"),
  arAmount: decimal("arAmount", { precision: 12, scale: 2 }).default("0.00"),
  roundingAdj: decimal("roundingAdj", { precision: 10, scale: 4 }).default("0.0000"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({
...
// ─── Location PINs (role-based access for procurement actions) ───
export const locationPins = mysqlTable("locationPins", {
  id: int("id").autoincrement().primaryKey(),
  locationId: int("locationId").notNull(),
  pinHash: varchar("pinHash", { length: 256 }).notNull(),
  label: varchar("pinLabel", { length: 128 }).notNull(),
  role: mysqlEnum("pinRole", ["manager", "ops_manager", "admin"]).notNull(),
  isActive: boolean("isActive").default(true).notNull(),
...
// ─── Inventory Levels (current stock per location per item) ───
export const inventoryLevels = mysqlTable("inventoryLevels", {
  id: int("id").autoincrement().primaryKey(),
  locationId: int("locationId").notNull(),
  inventoryItemId: int("inventoryItemId").notNull(),
  currentQty: decimal("currentQty", { precision: 12, scale: 3 }).default("0.000").notNull(),
  parLevel: decimal("locationParLevel", { precision: 10, scale: 2 }),
  reorderPoint: decimal("reorderPoint", { precision: 10, scale: 2 }),
  maxLevel: decimal("maxLevel", { precision: 10, scale: 2 }),
  avgDailyUsage: decimal("avgDailyUsage", { precision: 10, scale: 3 }).default("0.000"),
  lastCountDate: date("lastCountDate"),
  lastCountQty: decimal("lastCountQty", { precision: 12, scale: 3 }),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  uniqueLocationItem: unique("uniq_inv_level_loc_item").on(table.locationId, table.inventoryItemId),
}));

// ─── Stock Movements (ledger of all inventory changes) ───
export const stockMovements = mysqlTable("stockMovements", {
  ...
  quantity: decimal("movementQty", { precision: 12, scale: 3 }).notNull(),
  unitCost: decimal("movementUnitCost", { precision: 10, scale: 4 }),
  totalCost: decimal("movementTotalCost", { precision: 12, scale: 2 }),
  referenceType: varchar("referenceType", { length: 64 }),
  referenceId: int("referenceId"),
  notes: text("movementNotes"),
  reportedByPin: int("reportedByPin"),
  movementDate: date("movementDate").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});

// ─── Waste Reports (daily waste logging per location) ───
export const wasteReports = mysqlTable("wasteReports", {
  id: int("id").autoincrement().primaryKey(),
  locationId: int("locationId").notNull(),
  reportDate: date("reportDate").notNull(),
  reportedByPin: int("reportedByPin"),
  status: mysqlEnum("wasteReportStatus", ["draft", "submitted", "reviewed"]).default("draft").notNull(),
  totalWasteCost: decimal("totalWasteCost", { precision: 12, scale: 2 }).default("0.00"),
  reviewedBy: varchar("reviewedBy", { length: 256 }),
  reviewedAt: timestamp("reviewedAt"),
  notes: text("wasteReportNotes"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});

// ─── Waste Report Items (individual items in a waste report) ───
export const wasteReportItems = mysqlTable("wasteReportItems", {
  id: int("id").autoincrement().primaryKey(),
  wasteReportId: int("wasteReportId").notNull(),
  inventoryItemId: int("inventoryItemId").notNull(),
  quantity: decimal("wasteQty", { precision: 10, scale: 3 }).notNull(),
  unit: varchar("wasteUnit", { length: 32 }),
  reason: mysqlEnum("wasteReason", [
    "expired", "spoiled", "overproduction", "damaged",
    "quality_issue", "prep_waste", "customer_return", "other"
  ]).default("other").notNull(),
  estimatedCost: decimal("wasteEstimatedCost", { precision: 10, scale: 2 }),
  notes: text("wasteItemNotes"),
});

// ─── Leftover Reports (end-of-day leftover tracking) ───
export const leftoverReports = mysqlTable("leftoverReports", {
  id: int("id").autoincrement().primaryKey(),
  locationId: int("locationId").notNull(),
  reportDate: date("leftoverReportDate").notNull(),
  reportedByPin: int("leftoverReportedByPin"),
  status: mysqlEnum("leftoverReportStatus", ["draft", "submitted", "reviewed"]).default("draft").notNull(),
  notes: text("leftoverReportNotes"),
  createdAt: timestamp("leftoverCreatedAt").defaultNow().notNull(),
});

// ─── Leftover Report Items ───
export const leftoverReportItems = mysqlTable("leftoverReportItems", {
  id: int("id").autoincrement().primaryKey(),
  leftoverReportId: int("leftoverReportId").notNull(),
  inventoryItemId: int("inventoryItemId").notNull(),
  quantity: decimal("leftoverQty", { precision: 10, scale: 3 }).notNull(),
  unit: varchar("leftoverUnit", { length: 32 }),
  disposition: mysqlEnum("leftoverDisposition", [
    "carry_forward", "discount_sale", "staff_meal", "donate", "discard"
  ]).default("carry_forward").notNull(),
  estimatedCost: decimal("leftoverEstimatedCost", { precision: 10, scale: 2 }),
  notes: text("leftoverItemNotes"),
});

// ─── Vendor Catalog Items (products available from each vendor) ───
export const vendorCatalogItems = mysqlTable("vendorCatalogItems", {
  id: int("id").autoincrement().primaryKey(),
  supplierId: int("vendorCatalogSupplierId").notNull(),
  inventoryItemId: int("vendorCatalogInvItemId"),
  vendorSku: varchar("vendorSku", { length: 128 }),
  vendorProductName: varchar("vendorProductName", { length: 512 }).notNull(),
  vendorUnit: varchar("vendorUnit", { length: 64 }),
  vendorPackSize: varchar("vendorPackSize", { length: 64 }),
  unitPrice: decimal("vendorUnitPrice", { precision: 10, scale: 4 }),
  minOrderQty: decimal("vendorMinOrderQty", { precision: 10, scale: 2 }).default("1.00"),
  leadTimeDays: int("vendorLeadTimeDays").default(1),
  isAvailable: boolean("vendorItemAvailable").default(true),
  lastPriceUpdate: date("vendorLastPriceUpdate"),
  createdAt: timestamp("vendorCatalogCreatedAt").defaultNow().notNull(),
  updatedAt: timestamp("vendorCatalogUpdatedAt").defaultNow().onUpdateNow().notNull(),
});

// ─── Order Recommendations (smart ordering suggestions) ───
export const orderRecommendations = mysqlTable("orderRecommendations", {
  id: int("id").autoincrement().primaryKey(),
  locationId: int("recoLocationId").notNull(),
  inventoryItemId: int("recoInvItemId").notNull(),
  supplierId: int("recoSupplierId"),
  currentQty: decimal("recoCurrentQty", { precision: 12, scale: 3 }),
  parLevel: decimal("recoParLevel", { precision: 10, scale: 2 }),
  avgDailyUsage: decimal("recoAvgDailyUsage", { precision: 10, scale: 3 }),
  daysUntilStockout: int("recoDaysUntilStockout"),
  recommendedQty: decimal("recoRecommendedQty", { precision: 10, scale: 2 }),
  estimatedCost: decimal("recoEstimatedCost", { precision: 12, scale: 2 }),
  urgency: mysqlEnum("recoUrgency", ["critical", "high", "medium", "low"]).default("medium").notNull(),
  status: mysqlEnum("recoStatus", ["pending", "added_to_po", "dismissed"]).default("pending").notNull(),
