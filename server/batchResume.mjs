/**
 * Resume Batch Invoice Processing - 278 remaining invoices
 * 
 * Reads remaining_manifest.json (failed files from previous run),
 * processes each PDF with AI, creates invoice records.
 * 
 * Usage: node server/batchResume.mjs [--limit N] [--dry-run]
 */
...
async function invokeLLM(messages, responseFormat, retries = 2) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (resp.status === 412 && attempt < retries) {
      console.log(`    LLM quota hit, waiting 30s before retry (attempt ${attempt + 1}/${retries})...`);
...
// ─── MAIN ───
async function main() {
  console.log("=== Resume Batch Invoice Processor (278 remaining) ===");
  console.log(`Mode: ${dryRun ? "DRY RUN" : "LIVE"}`);
  console.log(`Limit: ${limit}`);

  const manifest = JSON.parse(fs.readFileSync("/home/ubuntu/invoice-batch/remaining_manifest.json", "utf-8"));
  const toProcess = manifest.slice(0, limit);
  
  console.log(`\nProcessing ${toProcess.length} remaining invoices...\n`);
  ...
  function saveProgress() {
    fs.writeFileSync("/home/ubuntu/invoice-batch/resume_results.json", JSON.stringify(results, null, 2));
  }
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    try {
      ...
      if (item.invoiceNumberFromFilename && item.supplier) {
        ...
        if (isDup) {
          console.log(`${progress} DUPLICATE: ${item.invoiceNumberFromFilename} - ${item.filename}`);
          results.duplicates++;
          continue;
        }
      }
      ...
      const invoiceNumber = extracted.invoiceNumber || item.invoiceNumberFromFilename || `BATCH-${Date.now()}-${i}`;
      ...
      if (results.processed % 25 === 0) {
        saveProgress();
        console.log(`  [Progress saved: ${results.created} created, ${results.errors} errors]`);
      }
      ...
      // If we hit quota exhaustion, save and stop
      if (err.message.includes("412") && err.message.includes("exhausted")) {
        console.log("\n⚠️  LLM quota exhausted again. Saving progress and stopping.");
        saveProgress();
        break;
      }
    }
  ...
  saveProgress();
  console.log("\nResults saved to /home/ubuntu/invoice-batch/resume_results.json");
